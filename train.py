...
def train(
    ...
    txt_output_path = "dummy_output.txt"
    with open(txt_output_path, "w") as f:
        f.write("This is a dummy output file.")
    
    # Create tar file containing the dummy output
    tar_output_path = "weights.tar"
    with tarfile.open(tar_output_path, "w") as tar:
        tar.add(txt_output_path)
    
    print(f"\nCreated tar file at {tar_output_path} containing {txt_output_path}")
    print("\n=== Test Complete ===")
    
    # Return the tar file path
    return TrainingOutput(weights=CogPath(tar_output_path))
